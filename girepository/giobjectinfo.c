...
/**
 * g_object_info_find_signal_using_interfaces:
 * @info: a #GIObjectInfo
 * @name: name of method to obtain
 * @implementor: (out) (transfer full): The implementor of the interface
 *
 * Obtain a signal of the object given a @name, searching both the
 * object @info and any interfaces it implements.  %NULL will be
 * returned if there's no method available with that name.
 *
 * Note that this function does *not* search parent classes; you will have
 ...
 */
GISignalInfo *
g_object_info_find_signal_using_interfaces (GIObjectInfo  *info,
					    const gchar   *name,
					    GIObjectInfo **implementor)
{
  GISignalInfo *result = NULL;
  GIObjectInfo *implementor_result = NULL;
  ...
  if (result == NULL)
      ...
      for (i = 0; i < n_interfaces; ++i)
	{
	  ...
	  result = g_interface_info_find_signal (iface_info, name);
